...
/**
 * This class generates and stores all the Gson deserializers. Each APIResource class can use
...
public class GsonFactory {
	...
	/**
	 * Shipment serializer uses a different deserializer so it can do special handling for addresses.
	 */
	public static final Gson SHIPMENT_GSON = new GsonBuilder()
		.setFieldNamingPolicy(FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES)
		.registerTypeAdapter(Shipment.class, new ShipmentDeserializer())
		.registerTypeAdapter(ShippoRawJsonObject.class,
			new ShippoRawJsonObjectDeserializer()).create();

	public static Gson getGson(Class className) {
		if (className == Shipment.class) {
